...
def make_user(event_date: date) -> dict:
    ...
    return {
        ...
        "avatar_url": f"https://cdn.example.com/avatars/{uid}.jpg"
...
def make_product(event_date: date) -> dict:
    ...
    return {
        ...
        "images": [
            f"https://cdn.example.com/products/{pid}-{i}.jpg"
...
def make_order(event_date: date) -> dict:
    ...
    return {
        ...
        "cancellation_reason": random.choice(
            ["out_of_stock", "user_request", "fraud", "payment_failed"]
        )
...
def make_review(event_date: date) -> dict:
    ...
    return {
        ...
        "images": [
            f"https://cdn.example.com/reviews/{rid}-{i}.jpg"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--rows", type=int, default=1_000_000, help="Nombre total d'items à générer"
    )
    p.add_argument(
        "--start", type=str, default="2024-01-01", help="Date de début YYYY-MM-DD"
    )
    p.add_argument(
        "--end", type=str, default="2024-12-31", help="Date de fin YYYY-MM-DD"
    )
    ...
    p.add_argument(
        "--seed", type=int, default=None, help="Seed pour la reproductibilité"
    )
    p.add_argument(
        "--chunk", type=int, default=CHUNK_SIZE, help="Taille des batchs mémoire"
    )
    return p.parse_args()
...
def main() -> None:
    ...
    with tqdm(
        total=args.rows, unit="items", unit_scale=True, dynamic_ncols=True
    ) as pbar:
        for day in days:
    ...
    # Résumé
    total_size = sum(f.stat().st_size for f in output.rglob(f"*.{ext}"))
