...
def main():
    spark = SparkSession.builder.appName("plain_spark") \
        .getOrCreate()
    config = Config()
    ...
    res = (
        ...
        .withColumn("delivery_duration_seconds",f.unix_timestamp("delivered_at") - f.unix_timestamp("placed_at"))
        .withColumn("delivery_delay_hour", f.round((f.unix_timestamp("delivered_at") - f.unix_timestamp("placed_at")) / 3600, 0).cast("int"))
        .withColumn("delivery_delay_sec", f.unix_timestamp("delivered_at") - f.unix_timestamp("placed_at"))
        .groupBy("cancellation_reason", "delivery_delay_hour")
        .agg(f.count("*").alias("count"), f.avg("delivery_delay_sec").alias("average_delivered_delay"))
    )
