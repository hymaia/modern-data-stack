...
def scan_all(source: Path) -> pl.LazyFrame:
    ...
    for p in partitions:
        ...
        lf = pl.scan_ndjson(p, infer_schema_length=10_000).with_columns(
            pl.lit(day).alias("dt")
        )
        frames.append(lf)
...
def enrich_orders(lf: pl.LazyFrame) -> pl.LazyFrame:
    return lf.with_columns(
        [
            pl.when(pl.col("total_amount") < 50)
            .then(pl.lit("low"))
            ...
            .alias("hour_of_day"),
            (
                pl.col("delivered_at").str.to_datetime(
                    "%Y-%m-%dT%H:%M:%SZ", strict=False
                )
                - pl.col("placed_at").str.to_datetime(
                    "%Y-%m-%dT%H:%M:%SZ", strict=False
                )
            )
            .dt.total_days()
...
def enrich_reviews(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    return lf.with_columns(
        [
            ...
            .then(
                pl.col("helpful_votes").cast(pl.Float64) / total_votes.cast(pl.Float64)
            )
            .otherwise(None)
...
def join_reviews_products(
    reviews: pl.LazyFrame, products: pl.LazyFrame
) -> pl.LazyFrame:
...
def join_orders_reviews(orders: pl.LazyFrame, reviews: pl.LazyFrame) -> pl.LazyFrame:
    ...
    order_items = (
        orders.select(["order_id", "dt", "items"])
        .explode("items")
        .with_columns(pl.col("items").struct.field("product_id").alias("product_id"))
        .select(["order_id", "product_id"])
    )

    review_agg = reviews.group_by("product_id").agg(
        [
...
def agg_product_rating(reviews: pl.LazyFrame) -> pl.LazyFrame:
    return (
        reviews.group_by(["dt", "product_id"])
        .agg(
            [
                pl.col("rating").mean().round(2).alias("avg_rating"),
...
def agg_user_basket(orders: pl.LazyFrame) -> pl.LazyFrame:
    return (
        orders.filter(pl.col("status").is_in(["delivered", "shipped", "processing"]))
        .group_by(["dt", "user_id"])
        .agg(
            [
